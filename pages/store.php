...
if ($myId >= 1) {
	if (filter_input(INPUT_POST, 'animalsUp', FILTER_SANITIZE_NUMBER_INT) == 1) {
		$newCategory = filter_input(INPUT_POST, 'category0', FILTER_SANITIZE_NUMBER_INT);
		$newCategoryNew = filter_input(INPUT_POST, 'categoryNew0', FILTER_SANITIZE_NUMBER_INT);
		$newSold = filter_input(INPUT_POST, 'sold0', FILTER_SANITIZE_NUMBER_INT);
		$newPrice = filter_input(INPUT_POST, 'price0', FILTER_SANITIZE_NUMBER_INT);
		$newName = htmlentities(filter_input(INPUT_POST, 'name0', FILTER_SANITIZE_NUMBER_INT), ENT_QUOTES);
		$newWeight = htmlentities(filter_input(INPUT_POST, 'weight0', FILTER_SANITIZE_NUMBER_INT), ENT_QUOTES);
		$newDescription = htmlentities(filter_input(INPUT_POST, 'description0', FILTER_SANITIZE_NUMBER_INT), ENT_QUOTES);
		if ($newName != "" && $newName != " ") {
			if ($newCategory == 0 && $newCategoryNew != "" && $newCategoryNew != " ") {
				$cat = getCategory($newCategoryNew, $db);
			} else {
				$cat = $newCategory;
			}
			$x = $db->prepare("INSERT INTO animals4sale VALUES(NULL,?,?,?,?,?,?,?,'0','0')");
			$x->execute(array(
					$cat,
					$newName,
					$newWeight,
			...
			$newId = $db->prepare("SELECT id FROM animals4sale WHERE name = ? ORDER BY id DESC LIMIT 1");
			$newId->execute(array(
					$newName
			));
			$newIdR = $newId->fetch();
			if ($newIdR) {
				$new = $newIdR['id'];
				if (isset($_FILES['image0']['tmp_name']) && $_FILES['image0']['size'] >= 1000) {
					$tmpFile = $_FILES['image0']["tmp_name"];
					list ($width1, $height1) = (getimagesize($tmpFile) != null) ? getimagesize($tmpFile) : null;
					if ($width1 != null && $height1 != null) {
						$image1Type = getPicType($_FILES['image0']['type']);
						$image1Name = $time . "." . $image1Type;
						processPic("store", $image1Name, $tmpFile, 800, 150);
						$p1stmt = $db->prepare("UPDATE animals4sale SET image = ? WHERE id = ?");
		...
		foreach ($_POST as $key => $val) {
			if (preg_match("/^category([0-9][0-9]*)$/", $key, $match) && $val >= 1) {
				$m = $match[1];
				if (filter_var($val, FILTER_VALIDATE_NUMBER_INT, FILTER_NULL_ON_FAILURE) && $m != 0) {
					$x = $db->prepare("UPDATE animals4sale SET category = ? WHERE id = ?");
					$x->execute(array(
							$val,
							$m
					));
			...
			if (preg_match("/^categoryNew([0-9][0-9]*)$/", $key, $match)) {
				$m = $match[1];
				if ($val != "" && $val != " ") {
					if (filter_var($val, FILTER_VALIDATE_STRING, FILTER_NULL_ON_FAILURE) && $m != 0) {
						$newId = getCategory($val, $db);
						$x2 = $db->prepare("UPDATE animals4sale SET category = ? WHERE id = ?");
						$x2->execute(array(
								$newId,
								$m
						));
			...
			if (preg_match("/^sold([0-9][0-9]*)$/", $key, $match)) {
				$m = $match[1];
				if (filter_var($val, FILTER_VALIDATE_NUMBER_INT, FILTER_NULL_ON_FAILURE) && $m != 0) {
					$x = $db->prepare("UPDATE animals4sale SET sold = ? WHERE id = ?");
					$x->execute(array(
							$val,
							$m
					));
			...
			if (preg_match("/^price([0-9][0-9]*)$/", $key, $match)) {
				$m = $match[1];
				if (filter_var($val, FILTER_VALIDATE_NUMBER_FLOAT, FILTER_NULL_ON_FAILURE) && $m != 0) {
					$x = $db->prepare("UPDATE animals4sale SET price = ? WHERE id = ?");
					$x->execute(array(
							$val,
							$m
					));
			...
			if (preg_match("/^name([0-9][0-9]*)$/", $key, $match)) {
				...
				if (filter_var($val, FILTER_VALIDATE_STRING, FILTER_NULL_ON_FAILURE) && $m != 0) {
					...
					$x = $db->prepare("UPDATE animals4sale SET name = ? WHERE id = ?");
					$x->execute(array(
							$v,
							$m
					));
			...
			if (preg_match("/^image([0-9][0-9]*)$/", $key, $match)) {
				$fullM = $match[0];
				$m = $match[1];
				if (isset($_FILES[$fullM]['tmp_name']) && $_FILES[$fullM]['size'] >= 1000 && $m != 0) {
			...
			if (preg_match("/^weight([0-9][0-9]*)$/", $key, $match)) {
				...
				if (filter_var($val, FILTER_VALIDATE_STRING, FILTER_NULL_ON_FAILURE) && $m != 0) {
					$v = htmlentities($val, ENT_QUOTES);
					$x = $db->prepare("UPDATE animals4sale SET weight = ? WHERE id = ?");
					$x->execute(array(
							$v,
							$m
					));
			...
			if (preg_match("/^description([0-9][0-9]*)$/", $key, $match)) {
				...
				if (filter_var($val, FILTER_VALIDATE_STRING, FILTER_NULL_ON_FAILURE) && $m != 0) {
					$v = htmlentities($val, ENT_QUOTES);
					$x = $db->prepare("UPDATE animals4sale SET description = ? WHERE id = ?");
					$x->execute(array(
							$v,
							$m
					));
		...
		foreach ($_POST as $key => $val) {
			if (preg_match("/^delAnimal([0-9][0-9]*)$/", $key, $match) && $val == 1) {
				$m = $match[1];
				if (filter_var($val, FILTER_VALIDATE_NUMBER_INT, FILTER_NULL_ON_FAILURE) && $m != 0) {
