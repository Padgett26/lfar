...
if (filter_input ( INPUT_POST, 'msgUp', FILTER_SANITIZE_NUMBER_INT ) == 1) {
	...
	$msg = htmlentities ( filter_input ( INPUT_POST, 'Message', FILTER_SANITIZE_STRING ), ENT_QUOTES );
...
if ($myId >= 1) {
	if (filter_input ( INPUT_POST, 'homePictures', FILTER_SANITIZE_NUMBER_INT ) == 1) {
		if (isset ( $_FILES ['imageTop'] ['tmp_name'] )) {
			$image1 = $_FILES ['imageTop'] ["tmp_name"];
			$folder = "pages";
			...
			if ($width1 != null && $height1 != null) {
				...
				$p1stmt = $db->prepare ( "UPDATE company SET homeImg1 = ? WHERE id = ?" );
		...
		if (isset ( $_FILES ['imageBottom'] ['tmp_name'] )) {
			$image2 = $_FILES ['imageBottom'] ["tmp_name"];
			$folder = "pages";
			...
			if ($width2 != null && $height2 != null) {
				...
				$p2stmt = $db->prepare ( "UPDATE company SET homeImg2 = ? WHERE id = ?" );
	...
	if (filter_input ( INPUT_POST, 'homeAbout', FILTER_SANITIZE_NUMBER_INT ) == 1) {
		if (isset ( $_FILES ['imageAbout'] ['tmp_name'] )) {
			$image1 = $_FILES ['imageAbout'] ["tmp_name"];
			$folder = "pages";
			list ( $width1, $height1 ) = (getimagesize ( $image1 ) != null) ? getimagesize ( $image1 ) : null;
			if ($width1 != null && $height1 != null) {
				...
				processPic ( $image1Name, $image1, $folder );
				processThumbPic ( $image1Name, $image1, $folder );
				$p1stmt = $db->prepare ( "UPDATE company SET aboutPic = ? WHERE id = ?" );
				$p1stmt->execute ( array (
						$image1Name,
						'1'
				) );
	...
	if (filter_input ( INPUT_POST, 'homeContact', FILTER_SANITIZE_NUMBER_INT ) == 1) {
		if (isset ( $_FILES ['imageContact'] ['tmp_name'] )) {
			$image1 = $_FILES ['imageContact'] ["tmp_name"];
			$folder = "pages";
			list ( $width1, $height1 ) = (getimagesize ( $image1 ) != null) ? getimagesize ( $image1 ) : null;
			if ($width1 != null && $height1 != null) {
				...
				processPic ( $image1Name, $image1, $folder );
				processThumbPic ( $image1Name, $image1, $folder );
				$p1stmt = $db->prepare ( "UPDATE company SET contactPic = ? WHERE id = ?" );
				$p1stmt->execute ( array (
						$image1Name,
						'1'
				) );
		...
		$p2stmt = $db->prepare ( "UPDATE company SET companyName = ?, address1 = ?, address2 = ?, phoneNumber = ?, email = ? WHERE id = ?" );
		$p2stmt->execute ( array (
				$cn,
				$a1,
				$a2,
