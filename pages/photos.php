...
<?php
$sDate = array ();
$getS = $db->prepare ( "SELECT DISTINCT showDate FROM photos ORDER BY showDate DESC" );
$getS->execute ();
...
if ($myId >= 1) {
	if (filter_input ( INPUT_POST, 'newpic', FILTER_SANITIZE_NUMBER_INT ) == 1) {
		$picShowDate = filter_input ( INPUT_POST, 'showDate', FILTER_SANITIZE_NUMBER_INT );
		$picCaption = htmlentities ( filter_input ( INPUT_POST, 'caption', FILTER_SANITIZE_STRING ), ENT_QUOTES );
		...
		$u2 = $db->prepare ( "SELECT id FROM photos WHERE caption = ? AND showDate = ? ORDER BY id DESC LIMIT 1" );
		$u2->execute ( array (
				$picCaption,
				$date
		) );
		...
		if (isset ( $_FILES ['pic'] ['tmp_name'] ) && $_FILES ['pic'] ['size'] >= 1000) {
			...
			if ($width1 != null && $height1 != null) {
				...
				$p1stmt = $db->prepare ( "UPDATE photos SET photo = ? WHERE id = ?" );
	...
	if (filter_input ( INPUT_POST, 'picUp', FILTER_SANITIZE_NUMBER_INT ) >= 1) {
		...
		if ($delpic == 1) {
			...
			$sDate = explode ( "-", $picShowDate );
			$date = mktime ( 12, 0, 0, $sDate [1], $sDate [2], $sDate [0] );
			$u = $db->prepare ( "UPDATE photos SET caption = ?, showDate = ? WHERE id = ?" );
			$u->execute ( array (
					$picCaption,
					$date,
					$picId
	...
	echo "<input type='date' name='showDate' value='" . date ( 'Y-m-d', $time ) . "'><br><br>\n";
	...
	foreach ( $sDate as $v ) {
		...
		echo "<div style='font-weight:bold; font-size:1.5em;'>" . date ( "Y-m-d", $v ) . "</div>\n";
